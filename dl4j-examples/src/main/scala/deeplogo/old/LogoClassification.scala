...
import org.deeplearning4j.examples.deeplogo.{MyImageRecordReader, MyUIServer}
...
object LogoClassification extends App{
  ...
  protected var channelsImageInNet = 3
  protected var numExamples = 2160
  protected var numLabels = 32
  protected var batchSize = 20
  protected var maxPathPerLabels = 60
  protected var seed = 15
  ...
  def customModel: MultiLayerNetwork = {
    ...
    val conf: MultiLayerConfiguration = new NeuralNetConfiguration.Builder()
      ...
      .optimizationAlgo(OptimizationAlgorithm.STOCHASTIC_GRADIENT_DESCENT).learningRate(0.1 * 1e-4).biasLearningRate(0.001 * 2)
      .learningRateDecayPolicy(LearningRatePolicy.Step).lrPolicyDecayRate(0.05).lrPolicySteps(100000)
      .regularization(true).l2(5 * 1e-4).momentum(0.90)//.miniBatch(false)
      .list
      .layer(0, new ConvolutionLayer.Builder(Array[Int](11, 11), Array[Int](4, 4), Array[Int](3, 3)).name("cnn1").nIn(channelsImageInNet).nOut(128).biasInit(0).activation(Activation.RELU).build)
      .layer(1, new SubsamplingLayer.Builder(PoolingType.MAX, Array[Int](3, 3), Array[Int](2, 2), Array[Int](1, 1)).name("maxpool1").build )
      .layer(2, new ConvolutionLayer.Builder(Array[Int](5, 5), Array[Int](2, 2), Array[Int](2, 2)).name("cnn3").nOut(64).biasInit(0).activation(Activation.RELU).build)
      //.layer(3, new ConvolutionLayer.Builder(Array[Int](5, 5), Array[Int](1, 1), Array[Int](2, 2)).name("cnn4").nOut(64).biasInit(0).activation(Activation.RELU).build)
      .layer(3, new SubsamplingLayer.Builder(PoolingType.MAX, Array[Int](3, 3), Array[Int](2, 2), Array[Int](1, 1)).name("maxpool1").build )
      .layer(4, fullyConnected("ffn1", 256, nonZeroBias, dropOut, new GaussianDistribution(0, 0.005)))
      ...
      .backprop(true)
      .pretrain(false)
      .setInputType(InputType.convolutional(height, width, channelsImageInNet))
      .build
  ...
  val mainPath = new File(System.getProperty("user.home"), "data/FlickrLogos-v2/classes/jpg2/")
  ...
  val transforms = Seq(flipTransform1)
...
*/
  ...
  def getImage(dataIter: RecordReaderDataSetIterator) = {
