...
import org.deeplearning4j.examples.convolution.{Vgg16, Vgg9}
...
/**
  ...
  */
class Vgg9Net(conf: Configuration) extends NetInterface{

  override def createNet(): MultiLayerNetwork = {
    val nonZeroBias: Double = 1
    val dropOut: Double = 0.5
    val networkConf: MultiLayerConfiguration = new Vgg9().conf(conf.width, conf.height, conf.iterations, conf.numLabels)
