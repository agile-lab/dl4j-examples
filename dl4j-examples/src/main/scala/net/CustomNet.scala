...
class CustomNet(conf: Configuration) extends NetInterface{
  ...
  override def createNet(): MultiLayerNetwork = {
    ...
    val networkConf: MultiLayerConfiguration = new NeuralNetConfiguration.Builder()
      ...
      .optimizationAlgo(OptimizationAlgorithm.STOCHASTIC_GRADIENT_DESCENT).learningRate(0.0007).biasLearningRate(0.00001 * 2)
      .learningRateDecayPolicy(LearningRatePolicy.Step).lrPolicyDecayRate(0.0002).lrPolicySteps(5000)
