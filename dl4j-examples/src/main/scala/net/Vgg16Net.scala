...
import org.deeplearning4j.examples.convolution.Vgg16
import org.deeplearning4j.examples.convolution.sentenceclassification.SimpleCNN
...
/**
  ...
  */
class Vgg16Net(conf: Configuration) extends NetInterface{

  override def createNet(): MultiLayerNetwork = {
    val nonZeroBias: Double = 1
    val dropOut: Double = 0.5
    val networkConf: MultiLayerConfiguration = new Vgg16().conf(conf.width, conf.height, conf.iterations, conf.numLabels)
