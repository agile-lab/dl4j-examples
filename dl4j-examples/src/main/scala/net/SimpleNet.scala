...
/**
  ...
  */
class SimpleNet(conf: Configuration) extends NetInterface{

  override def createNet(): MultiLayerNetwork = {
    val nonZeroBias: Double = 1
    val dropOut: Double = 0.5
    val networkConf: MultiLayerConfiguration = new SimpleCNN().conf(conf.width, conf.height, conf.iterations, conf.numLabels)
