    ...
    public MultiLayerConfiguration conf(int width, int height, int iterations, int numLabels) {
        ...
        MultiLayerConfiguration conf =
            ...
            new NeuralNetConfiguration.Builder()
                ...
                .layer(5, new SubsamplingLayer.Builder().poolingType(SubsamplingLayer.PoolingType.MAX).kernelSize(2, 2).stride(2, 2).build())

                // block 3

                .layer(6, new ConvolutionLayer.Builder().kernelSize(3, 3).stride(1, 1).padding(1, 1).nOut(256).cudnnAlgoMode(ConvolutionLayer.AlgoMode.PREFER_FASTEST).build())

                .layer(7, new ConvolutionLayer.Builder().kernelSize(3, 3).stride(1, 1).padding(1, 1).nOut(256).cudnnAlgoMode(ConvolutionLayer.AlgoMode.PREFER_FASTEST).build())
