 ...
 * @author Adam Gibson
 */
public class MyMultiLayerNetwork implements Serializable, Classifier, Layer {
    private static final Logger log = LoggerFactory.getLogger(MyMultiLayerNetwork.class);
     ...
     */
    public MyMultiLayerNetwork(String conf, INDArray params) {
    ...
    public void updateRnnStateWithTBPTTState() {
        for (int i = 0; i < layers.length; i++) {
            ...
            } else if (layers[i] instanceof MyMultiLayerNetwork) {
                ((MyMultiLayerNetwork) layers[i]).updateRnnStateWithTBPTTState();
            }
        }
    }
     ...
     */
    public void update(MyMultiLayerNetwork network) {
        ...
        if (network.solver != null) {
            ...
            if (updaterView != null) {
                Updater newUpdater = new MyMultiLayerUpdater(this, updaterView.dup());
     ...
     */
    @Deprecated
    public void merge(MyMultiLayerNetwork network, int batchSize) {
    ...
    public INDArray rnnTimeStep(INDArray input) {
        ...
        for (int i = 0; i < layers.length; i++) {
            ...
            if (layers[i] instanceof RecurrentLayer) {
                ...
                input = ((MyMultiLayerNetwork) layers[i]).rnnTimeStep(input);
            } else {
    ...
    public void rnnClearPreviousState() {
        ...
        for (int i = 0; i < layers.length; i++) {
            if (layers[i] instanceof RecurrentLayer)
                ...
                ((MyMultiLayerNetwork) layers[i]).rnnClearPreviousState();
            }
        }
    }
    ...
    public List<INDArray> rnnActivateUsingStoredState(INDArray input, boolean training, boolean storeLastForTBPTT) {
        ...
        for (int i = 0; i < layers.length; i++) {
            ...
            if (layers[i] instanceof RecurrentLayer) {
                ...
                List<INDArray> temp = ((MyMultiLayerNetwork) layers[i]).rnnActivateUsingStoredState(currInput, training,
                    storeLastForTBPTT);
